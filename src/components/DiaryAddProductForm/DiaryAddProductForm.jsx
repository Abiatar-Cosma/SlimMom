...
const DiaryAddProductForm = () => {
  ...
  const selectOptions = productsOptions.map((i) => {
    if (
      userDailyDiet?.notAllowedProduct?.find((item) => item.title === i.title)
    ) {
      return {
        value: i.title,
        label: `${i.title} *`,
      ...
      <form className={s.form} onSubmit={handleFormSubmit}>
        {clarification && (
          <p className={s.clarification}>
            * Items on the orange background in the drop down list are not
            recommended for your diet.
          </p>
        )}
        <label className={s.label}>
          <Select
            ...
            onMenuClose={() => setClarification(false)}
            // for styles
            // unstyled
